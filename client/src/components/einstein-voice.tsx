...
// API function for interacting with ElevenLabs
async function getEinsteinResponse(message: string): Promise<string> {
  try {
    const response = await fetch('https://api.elevenlabs.io/v1/conversation', {
      ...
      body: JSON.stringify({
        text: message,
        agent_id: "hWQi3k8Rnma2afkUtNKE" // Einstein agent ID
      })
    });
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognitionInstance.onresult = async (event: any) => {
        ...
        try {
          const response = await getEinsteinResponse(transcript);
